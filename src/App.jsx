...
async function mockNlpApi(query, scenarioId) {
  ...
  // Returns structure: { text, cards: [], topics: [], details: {} }
  ...
  if (
    ...
    if (scenarioId === "home") {
      return {
        ...
        // No details/transparency for greeting to keep it clean
      };
    } else {
      ...
      return {
        ...
        cards: rich.cards,
        topics: rich.topics,
        // No details/transparency for greeting
      };
    }
  }
  ...
  if (response) {
    if (
      ...
      if (context === "tax") {
        response.text =
          "Filing a tax return involves three main steps. First, gather your income documents like W-2s and 1099s. Next, choose a filing method such as e-filing or mailing paper forms. Finally, submit your return and track your refund status online.";
        response.cards = [
          {
            title: "E-File Online",
    ...
    // Add logic details for Prototype B (Transparency) ONLY for query responses
    if (!response.details) {
      response.details = {
  ...
  return (
    <div className="flex justify-start mb-6 gap-3">
      ...
      <div className="flex flex-col gap-3 max-w-[95%] w-full">
        ...
        {/* 4. Prototype B Footer (Transparency) */}
        {/* Only render this if details exist - Removed for welcome message */}
        {data.details && (
...
export default function App() {
  ...
  const handleSend = async (e, textOverride) => {
    // FIX: preventDefault is method of event object 'e', not textOverride
    if (e && e.preventDefault) e.preventDefault();

    // If textOverride is a string (from card click), use it. Otherwise use input state.
